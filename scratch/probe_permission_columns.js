...
async function probe() {
  console.log('Probing permission table...')
  const { data, error } = await supabase.from('permission').select('*').limit(1)
  if (error) {
    console.error('Error:', error)
  } else {
    console.log('Permission record keys:', Object.keys(data[0] || {}))
